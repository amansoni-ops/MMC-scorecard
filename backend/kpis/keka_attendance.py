    ...
    for kid, rows in by_emp.items():
        ...
        for r in rows:
            if not r.get('is_early_exit'):
                continue
            m_out_str = modal_shift.get(kid, (None, None))[1]
            shift_end_dt = (datetime.combine(r['date'], datetime.strptime(m_out_str, '%H:%M').time())
                            if m_out_str else None)
            punch_out = r.get('punch_out')

            # REWORKED per explicit request: no more decimal-hours math
            # against a hardcoded 8.5h expectation. Expected = the
            # person's real modal shift-end clock time. Actual = their
            # real punch-out clock time. Shortfall = how much TIME
            # (hours + minutes, not a decimal) before shift_end they
            # actually left — i.e. shift_end - punch_out, only when
            # punch_out is genuinely earlier than shift_end.
            shortfall_h, shortfall_m = None, None
            if shift_end_dt and punch_out and punch_out < shift_end_dt:
                total_seconds = (shift_end_dt - punch_out).total_seconds()
                total_minutes = int(round(total_seconds / 60))
                shortfall_h, shortfall_m = divmod(total_minutes, 60)

            early_entries.append({
                'date': str(r['date']),
                'punch_out': _fmt_dt(punch_out),
                'shift_end': _fmt_dt(shift_end_dt),
                'shortfall_hours': shortfall_h,
                'shortfall_minutes': shortfall_m,
            })
    ...
    def fetch(self, month, year):
        ...
        for emp in data['employees']:
            ...
            rows.append({
                ...
                'KekaID': emp['keka_id'],
                # NEW: surfaces this month's join/exit info to the UI —
                # this data was already computed internally for the
                # working-days proration math, just never exposed before.
                'JoinedMidMonth': emp['joined_mid_month'],
                'JoiningDate': emp['joining_date'],
                'ExitedMidMonth': emp['exited_mid_month'],
    ...
    def aggregate(self, rows):
        ...
        return {
            ...
            'orders': [{'working_days': wdays, 'present_days': present,
                        ...
                        'joined_mid_month': r['JoinedMidMonth'], 'joining_date': r['JoiningDate'],
                        'exited_mid_month': r['ExitedMidMonth'], 'exit_date': r['ExitDate']}],
        }
