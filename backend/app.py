...
@app.get('/api/scores/<int(signed=True):admin_id>')
@_auth_required
def employee_score(admin_id):
    now=datetime.now()
    month=int(request.args.get('month',now.month))
    year =int(request.args.get('year', now.year))
    # FAST PATH: reads pre-computed per-KPI rows from employee_kpi_cache
    # instead of recomputing every KPI for every employee on each click.
    # Falls back to the original score_engine.get_employee_detail()
    # automatically if this employee/month has no cached rows yet.
    detail=score_engine.get_employee_detail_fast(admin_id,month,year)
    if not detail: return jsonify({'error':'Not found'}),404
    detail['note']=local_db.get_note(admin_id,month,year)
    return jsonify(detail)

@app.get('/api/scores/<int(signed=True):admin_id>/trend')
@_auth_required
def employee_trend(admin_id):
    now=datetime.now()
    year=int(request.args.get('year',now.year))
    # FAST PATH: reads employee_kpi_cache directly instead of looping
    # score_cache.load() for all 12 months (which forced a full
    # recompute for any uncached month — the root cause of the trend
    # chart showing no data for most months).
    fast_trend = score_engine.get_employee_trend_fast(admin_id, year)
    trend=[]
    for entry in fast_trend:
        if year==now.year and entry['month']>now.month:
            break
        trend.append({'month':entry['month'],'year':year,
            'total_score':entry['total_score'],
            'tier':None,'kpis':{}})
    return jsonify({'admin_id':admin_id,'year':year,'trend':trend})

# ── Employee cache refresh (single employee, on demand) ───────────────────
@app.post('/api/employee/<int(signed=True):admin_id>/refresh')
@_auth_required
@_admin_required
def refresh_employee_route(admin_id):
    now=datetime.now()
    month=int(request.args.get('month',now.month))
    year =int(request.args.get('year', now.year))
    try:
        import populate_employee_cache as pec
        pec.refresh_one_employee(admin_id, month, year)
        return jsonify({'ok':True,'admin_id':admin_id,'month':month,'year':year})
...
@app.get('/api/independence/<int(signed=True):admin_id>')
@_auth_required
...
@app.put('/api/independence/<int(signed=True):admin_id>')
@_auth_required
@_admin_required
...
# NOTE: the four /api/keka/... routes that previously appeared here
# (keka_attendance, keka_employee_attendance, keka_active_users,
# keka_refresh) called get_attendance_data / get_employee_attendance /
# get_active_users_count / refresh_attendance_cache — functions from an
# EARLIER version of kpis/keka_attendance.py that no longer exist after
# it was rewritten into KekaLeavesKPI / KekaLateComingsKPI /
# KekaEarlyLeavingsKPI. Nothing in the current frontend calls these
# routes, so they were removed rather than left in a broken,
# import-error state. If a standalone Keka attendance view is needed
# later, these would need to be rewritten against the current
# kpis/keka_attendance.py interface (get_month_data, etc.), not restored
# as-is.
...
# ── Notes & Cache ─────────────────────────────────────────────────────────
@app.get('/api/notes/<int(signed=True):admin_id>')
@_auth_required
...
@app.post('/api/notes/<int(signed=True):admin_id>')
@_auth_required
...
# # ── Employee cache refresh (single employee, on demand) ───────────────────
# @app.post('/api/employee/<int:admin_id>/refresh')
# @_auth_required
# @_admin_required
# def refresh_employee_route(admin_id):
#     now=datetime.now()
