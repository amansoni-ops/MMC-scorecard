...
def get_all_users() -> list[dict]:
    """Never returns password hashes — listing users should not expose them."""
    with _conn() as c:
        rows = c.execute('SELECT id, username, role FROM users ORDER BY username').fetchall()
    return [dict(r) for r in rows]

def create_user(username: str, password: str, role: str = 'viewer') -> dict:
    """Raises ValueError on duplicate username or invalid input, instead
    of letting a raw sqlite3.IntegrityError leak up to the API layer."""
...
def change_password(user_id: int, new_password: str):
    ...
    with _conn() as c:
        cur = c.execute('UPDATE users SET password=? WHERE id=?', (pw, user_id))
        if cur.rowcount == 0:
            raise ValueError(f'No user with id={user_id}')

def change_password_by_username(username: str, new_password: str):
    """Used for self-service 'change my own password', where the caller
    only knows their username (from session), not their numeric id."""
    if not new_password or len(new_password) < 6:
        raise ValueError('Password must be at least 6 characters')
    pw = hashlib.sha256(new_password.encode()).hexdigest()
    with _conn() as c:
        cur = c.execute('UPDATE users SET password=? WHERE username=?', (pw, username))
        if cur.rowcount == 0:
            raise ValueError(f'No user "{username}"')

def delete_user(user_id: int, requesting_username: str):
    """Refuses to let an admin delete their own account via this path —
    a safety guard against accidentally locking yourself out with no
    other admin account to recover with."""
    with _conn() as c:
        row = c.execute('SELECT username FROM users WHERE id=?', (user_id,)).fetchone()
        if not row:
            raise ValueError(f'No user with id={user_id}')
        if row['username'] == requesting_username:
            raise ValueError('Cannot delete your own account while logged in as it')
        admin_count = c.execute("SELECT COUNT(*) AS cnt FROM users WHERE role='admin'").fetchone()['cnt']
        target_role = c.execute('SELECT role FROM users WHERE id=?', (user_id,)).fetchone()['role']
