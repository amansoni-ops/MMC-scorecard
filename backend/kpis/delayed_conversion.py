...
BaseData AS (
    SELECT m.ID_OrderDetailsMisc, m.OrderID, m.PickedUpBy, m.Summary_AssignUserID,
        ...
        -- FIXED: same Reckon auto-completion fix as post_conversion.py —
        -- confirmed via real data that Reckon files auto-complete exactly
        -- 7 days after Date_OrderReviewStatus with no human action, making
        -- od.CompletionOn meaningless for delay calculation on these files.
        -- Using Date_OrderReviewStatus as the effective completion date for
        -- Reckon files specifically; every other platform unaffected.
        CAST(
            CASE
