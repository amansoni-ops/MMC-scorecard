...
def _match_one(keka_name, admin_rows):
    ...
    if len(exact) > 1:
        # FIX: before giving up as ambiguous, check if exactly ONE
        # candidate is Active=True — confirmed safe via real
        # investigation this session (investigate_ambiguous_names.py):
        # ~37% of ambiguous-name cases resolve cleanly this way, with
        # zero risk of misattribution, since every duplicate-name
        # situation found so far has the pattern "1 real active
        # employee + N old inactive/dead accounts from past platform-
        # specific signups". Confirmed concretely for Preeti Shinde:
        # AdminID=19 (Active=True) vs AdminID=347 (Active=False) — the
        # correct answer was always unambiguous by activity status,
        # even though name-only matching correctly couldn't tell them
        # apart. Falls through to ambiguous_exact (unchanged, safe
        # default) if zero or 2+ candidates are Active=True.
        active_candidates = [a for a in exact if a.get('Flag_Active') and not a.get('Flag_Delete')]
        if len(active_candidates) == 1:
            return active_candidates[0]['AdminID'], active_candidates[0]['AdminName'], 'exact_active_disambiguated'
