...
def _run_loop():
    ...
    while not _stop_flag.is_set():
        try:
            ...
            # FIX: previously looped through ALL 12 months unconditionally,
            # which wastes real time (confirmed: ~57s per month, even for
            # months with zero possible data) fetching/computing/writing
            # nothing for months that haven't happened yet. Future months
            # genuinely cannot have any real Keka attendance or SQL Server
            # conversion data — there's nothing to populate. Stop at the
            # current month for the current year; this doesn't affect
            # populating a genuinely PAST year, where all 12 months are
            # real and complete.
...
def run_now(year=None):
    ...
    For the CURRENT year, stops at the current month (future months have
    no possible data). For a PAST year passed explicitly, populates all
    12 months since they're all genuinely real and complete."""
    import populate_employee_cache as pec
    now = datetime.utcnow()
