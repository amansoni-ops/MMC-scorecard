...
ResolvedAttribution AS (
    ...
    WHERE od.Flag_Deleted = 0
      -- FIX (Reckon auto-completion, same as post_conversion.py /
      -- delayed_conversion.py — confirmed via Rosebud Property,
      -- review=2026-05-31, auto-completed 2026-06-07, gap=7 days):
      -- for Reckon files, the real review date is the meaningful
      -- completion event, not the +7-day automated trigger.
      AND (CASE
...
FilesCompleted AS (
    SELECT
        ...
        od.CompanyName,
        -- FIX (Reckon auto-completion — same pattern as post_conversion.py
        -- / delayed_conversion.py / the aggregate query above): for Reckon
        -- files, use the real customer review date as the effective
        -- completion date, not the automated +7-day trigger.
        CAST(
            CASE
                WHEN od.DestinationSW = 'Reckon' AND m.Date_OrderReviewStatus IS NOT NULL
                THEN m.Date_OrderReviewStatus
        ...
        CASE
            WHEN fad.FirstDate IS NOT NULL
             AND CAST(fad.FirstDate AS DATE) <= CAST(
                    CASE
                        WHEN od.DestinationSW = 'Reckon' AND m.Date_OrderReviewStatus IS NOT NULL
                        THEN m.Date_OrderReviewStatus
                        ELSE od.CompletionOn
...
-- FIX (confirmed real bug via Motorlife Museum, OrderID=34803): when a
-- file is ALLOCATED and COMPLETED on the SAME calendar day,
-- ActiveFrom = that day, but ActiveTo (per the fix above) becomes the
-- working day BEFORE it — i.e. ActiveTo < ActiveFrom. The previous
-- INNER JOIN to WorkingDays then matched ZERO rows for ANY date
-- (since the range is inverted/empty), causing the file to silently
-- vanish from the report entirely instead of correctly showing 0
-- active days (a same-day turnaround genuinely has no days where an
-- update was "missed" — there was no time for one). Switched to LEFT
-- JOIN + COALESCE so these files now appear with ActiveWorkingDays=0
-- rather than not appearing at all. Every file with a normal
-- multi-day window is completely unaffected by this change.
FileActiveDays AS (
    SELECT
        f.AdminID, f.OrderID, f.OrderNumber,
        f.CompanyName, f.CompletedDate, f.ActiveFromRaw AS ActiveFrom, f.ActiveTo,
        COALESCE(COUNT(wd.WorkDay), 0) AS ActiveWorkingDays
